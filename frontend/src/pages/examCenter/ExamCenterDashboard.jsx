...
import KeyManager from '../../components/examCenter/KeyManager';
...
const ExamCenterDashboard = () => {
  ...
  const handleDownload = async (paper) => {
    ...
    if (!privKey) {
      alert('Private key not found! Please generate/restore your keys first.');
      return;
    }
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-1 space-y-6">
        <KeyManager onRegisterSuccess={fetchPapers} />
        
        {status && (
          <div className="glass-card p-4 bg-blue-500/10 border-blue-500/20 text-blue-500 text-sm animate-pulse">
            {status}
          </div>
        )}
      </div>
